@echo off
REM con1 connector - server install script
...
if /I not "%SKIPREGISTER%"=="true" call infasetup validateAndRegisterFeature -ff "%INFA_HOME%/plugins/conf/heston.con1.xml" -up %upgrade%	

rmdir /s /q "%INFA_HOME%/services/work_dir"
rmdir /s /q "%INFA_HOME%/tomcat/bin/workspace/.metadata"
